...
function invalid(kind: 'backup' | 'session', detail: string): never {
  const label = kind === 'backup' ? 'backup' : 'session JSON';
  throw new Error(`That ${label} is invalid (${detail}). Nothing was changed.`);
}

function record(value: unknown, kind: 'backup' | 'session', path: string): JsonRecord {
  if (!isRecord(value)) invalid(kind, `${path} must be an object`);
  return value;
}

function text(value: unknown, kind: 'backup' | 'session', path: string): string {
  if (typeof value !== 'string' || !value.trim()) invalid(kind, `${path} is missing`);
  return value;
}

function number(value: unknown, kind: 'backup' | 'session', path: string, min: number, max = Number.MAX_SAFE_INTEGER): number {
  if (typeof value !== 'number' || !Number.isFinite(value) || value < min || value > max) invalid(kind, `${path} is out of range`);
  return value;
}

function integer(value: unknown, kind: 'backup' | 'session', path: string, min: number, max: number): number {
  const parsed = number(value, kind, path, min, max);
  if (!Number.isInteger(parsed)) invalid(kind, `${path} must be a whole number`);
  return parsed;
}

function note(value: unknown, kind: 'backup' | 'session', path: string): NoteEvent {
...
function pedal(value: unknown, kind: 'backup' | 'session', path: string): PedalEvent {
  const input = record(value, kind, path);
...
function cleanedNote(value: unknown, kind: 'backup' | 'session', path: string): CleanedNote {
  const input = record(value, kind, path);
 ...
 */
export function validateImportedTake(value: unknown, kind: 'backup' | 'session' = 'session'): Take {
